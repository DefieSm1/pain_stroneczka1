	...
	<div class="delta-container">
		...
		<form action="?" method="post">
			<div class="form-inputs">
				...
				<input type="password" name="password_confirm" id="password_confirm_input" placeholder="Potwierdź hasło" required>
			...
			if(isset($_POST["isSubmitted"]) && $_POST["isSubmitted"] == "true")
			{
				if($_POST["username"] != "" && $_POST["password"] != "") {
					if(!filter_var($_POST["email"], FILTER_VALIDATE_EMAIL))
						echo '<p>Umiesz imejl wpisywać?</p>';
					else if($_POST["password"] != $_POST["password_confirm"])
						echo '<p>Hasła nie powinny się różnić!</p>';
					else {
						...
						if($conn->query("SELECT * FROM users WHERE username = '".$username."'")->num_rows > 0) {
							echo '<p>Już taki użytkownik istnieje, podaj nową nazwę użytkownika</p>';
						}
							...
							echo '<p>Zarejestrowano użytkownika '.$username;
						}
